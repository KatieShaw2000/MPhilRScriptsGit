...
#join SLA and hyperspectral data together ----

all <- merge(SLA, hyperspectral, by = c("Plot", "Repeat"))
rm(SLA, hyperspectral)

all <- all[order(all$SLA),]
...
output_dir <- ("~/OneDrive - University of Cambridge/MPhil/Phenotyping Campaign/Hyperspectral files/SLA modelling")
